...
runCommand() {
	...
	if [ "$command" == 'update' ]; then
		...
		echo "Usage: $runPath [start|connect|input|output|clean|restart|stop|count|started|running|logs|active] [-connect true|false] [-output on|off] [-service true|false]"
		exit 1
	fi
